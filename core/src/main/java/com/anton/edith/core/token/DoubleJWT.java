package com.anton.edith.core.token;

import com.anton.edith.core.constant.TokenConstant;
import com.anton.edith.core.util.DateUtil;
...
public class DoubleJWT {
    ...
    private void checkTokenScope(String scope) {
        if (scope == null || !scope.equals(TokenConstant.EDITH_SCOPE)) {
    ...
    public String generateAccessToken(long identity) {
        return generateToken(TokenConstant.ACCESS_TYPE, identity, TokenConstant.EDITH_SCOPE, this.accessExpire);
    }

    public String generateRefreshToken(long identity) {
        return generateToken(TokenConstant.REFRESH_TYPE, identity, TokenConstant.EDITH_SCOPE, this.refreshExpire);
    }

    public Tokens generateTokens(long identity) {
        String access = this.generateToken(TokenConstant.ACCESS_TYPE, identity, TokenConstant.EDITH_SCOPE, this.accessExpire);
        String refresh = this.generateToken(TokenConstant.REFRESH_TYPE, identity, TokenConstant.EDITH_SCOPE, this.refreshExpire);
